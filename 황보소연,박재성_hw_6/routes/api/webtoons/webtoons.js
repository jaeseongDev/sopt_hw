...
router.post('/', upload.array('thumbnail'), async(req, res) => {
    try {
        ...
        // 사진이 2장 이상 첨부된 경우
            ...
            let query = 'INSERT INTO webtoons (thumbnail, title, writer, isFinished) VALUES (?, ?, ?, 0)';
            await connection.query(query, [thumbnail, title, writer]);
...
router.delete('/', async(req, res) => {
    try {
        ...
        if (!webtoonsIdx) {
            ...
            let query = 'DELETE FROM webtoons WHERE webtoonsIdx = ?';
